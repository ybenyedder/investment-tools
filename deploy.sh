...
sshpass -p "$PASSWORD" rsync -avz -e "ssh -o StrictHostKeyChecking=no" --exclude '.git' --exclude 'node_modules' --exclude 'venv' --exclude '.next' --exclude '__pycache__' --exclude 'whatsapp-bot' --exclude '*.db' --exclude 'chroma_db' ./ $SERVER_USER@$SERVER_IP:$PROJECT_DIR/
...
sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no $SERVER_USER@$SERVER_IP "cd $PROJECT_DIR && docker compose down --remove-orphans || true && docker compose up -d --build"
